...
class BotState:
    def __init__(self):
        ...
        self.waiting_for_channel = False  # ✅ добавлено
...
# ✅ команда смены канала
async def set_channel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    bot_state.waiting_for_channel = True
    await update.message.reply_text("Отправь новый CHANNEL_ID (пример: -1001234567890)")

# ✅ обработка ввода
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if bot_state.waiting_for_channel:
        new_id = update.message.text.strip()
        try:
            CONFIG["CHANNEL_ID"] = int(new_id)
            bot_state.waiting_for_channel = False
            await update.message.reply_text(f"CHANNEL_ID обновлён: {new_id}")
        except:
